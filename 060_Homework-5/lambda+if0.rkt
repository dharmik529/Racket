  ...
  (cond
    ...
    [(s-exp-match? `{letrec {[SYMBOL ANY]} ANY} s)
     (let ([bs (s-exp->list (first
       ...
       (appE (lamE (s-exp->symbol (first bs))
                   (parse (third (s-exp->list s))))
             (appE (parse mk-rec-fun)(lamE (s-exp->symbol (first bs))
                                           (parse (second bs))))))]
    [(s-exp-match? `{ANY ANY} s)
...
(define times
  `{lambda (n) {lambda (m) (if (or (not (integer? n)) (not (integer? m)) (< n 0) (< m 0))
                               (error "Invalid arguments")
                               (numV (* n m)))}})

(test (interp (parse `{letrec {[f {lambda {n} {if0 n 0 {+ {f {+ n 1}} 1}}}]} {f -7}}) mt-env) (numV 7))
